"""Tests for `CSVExportService` (Phase 6.3, ADR-039).

Uses an in-memory `PredictionHistoryRepository` implementation so these
tests exercise only `CSVExportService`'s own orchestration -- validation,
delegation to the repository, delegation to `PredictionAnalyticsService`,
and delegation to `CSVExportBuilder` -- never a real database.
"""
...
class TestCSVExportServiceExport:
    """Verifies `CSVExportService.export_csv()` orchestration."""

    def test_exports_csv_scoped_to_the_requesting_user(
    ...
    def test_user_with_no_history_receives_a_zero_row_result(
        ...
        result = asyncio.run(service.export_csv(user_id="user-nobody"))
    ...
    def test_invalid_request_raises_before_touching_the_repository(self) -> None:
        ...
        with pytest.raises(InvalidCSVExportRequestError):
            asyncio.run(service.export_csv(user_id=""))
    ...
    def test_filters_are_passed_through_to_the_repository_unchanged(
        ...
        asyncio.run(service.export_csv(user_id="user-0001", filters=filters))
