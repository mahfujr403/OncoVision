"""Tests for `PredictionAnalyticsService` (Phase 6.2, ADR-038).

Uses an in-memory `PredictionHistoryRepository` implementation so these
tests exercise only `PredictionAnalyticsService`'s own orchestration --
validation, delegation to the repository, and delegation to
`AnalyticsBuilder` -- never a real database.
"""
...
class TestPredictionAnalyticsServiceComputation:
    """Verifies `PredictionAnalyticsService.compute_analytics()` orchestration."""

    def test_computes_analytics_scoped_to_the_requesting_user(
        self, history_records: list[PredictionHistory]
    ) -> None:
        repository = InMemoryPredictionHistoryRepository(history_records)
        service = PredictionAnalyticsService(history_repository=repository)
    ...
    def test_user_with_no_history_receives_an_empty_result(
        self, history_records: list[PredictionHistory]
    ) -> None:
        repository = InMemoryPredictionHistoryRepository(history_records)
        service = PredictionAnalyticsService(history_repository=repository)

        result = asyncio.run(service.compute_analytics(user_id="user-nobody"))
    ...
    def test_invalid_request_raises_before_touching_the_repository(self) -> None:
        ...
        with pytest.raises(InvalidAnalyticsRequestError):
            asyncio.run(service.compute_analytics(user_id=""))
    ...
    def test_filters_are_passed_through_to_the_repository_unchanged(
        ...
        asyncio.run(service.compute_analytics(user_id="user-0001", filters=filters))
