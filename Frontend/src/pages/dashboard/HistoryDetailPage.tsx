  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" onClick={() => navigate(ROUTES.HISTORY)}>
      ...
      {!isLoading && notFound && (
        <Card>
          <EmptyState
            ...
            description="This prediction history record doesn't exist or you don't have access to it."
            action={{ label: 'Back to history', onClick: () => navigate(ROUTES.HISTORY) }}
          />
        </Card>
      )}
        ...
        <>
          ...
          <Card>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {[
                { label: 'Filename', value: record.image_metadata.filename },
                { label: 'Content Type', value: record.image_metadata.content_type },
                { label: 'File Size', value: formatFileSize(record.image_metadata.size_bytes) },
                {
                  label: 'Dimensions',
                  value: `${record.image_metadata.width} × ${record.image_metadata.height}`,
                },
